...
"""))
		if action == 1:
			...
			anki_engine.card.create(user_id, side1, side2)
		elif action == 2:
			...
			anki_engine.label.create(user_id, input('Введите название заголовка: ').strip(), flag)
		elif action == 4:
			...
			anki_engine.cardLabelRelation.create(user_id, card_id, label_id)
		elif action == 6:
			...
			label_id = int(input('Введите ID заголовка: '))
			label = anki_engine.utils.empty_protected_read(anki_engine.Label, label_id)
			train = anki_engine.get_cards_to_train(label)
			i = 0
			for train_card in train:
				q = int(input(f'{train_card[0]} - Оцени уровень памяти от 0 до 5: '))
				anki_engine.recalculate_memory_note(i, q)
				i += 1
			if len(train) == 0:
				print('Нечего тренировать, можете отдохнуть )')
		elif action == 9:
			anki_engine.card.delete(user_id, int(input('Введите индекс удаляемой карточки? ')))
		elif action == 10:
			anki_engine.label.delete(user_id, int(input('Введите индекс удаляемого заголовка? ')))
		else:
			break
